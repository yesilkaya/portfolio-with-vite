...
import { ContactUser } from "../src/types/user.js";
...
const server = http.createServer((req, res) => {
  ...
  (async () => {
    ...
    if (req.method === "GET" && pathname === "/api/feedback") {
      try {
        const rows = await db.all<ContactUser[]>("SELECT * FROM contact");
    ...
    // POST: Yeni kayıt
      ...
      try {
        const { first_name, last_name, email, message } = await parseRequestBody<ContactUser>(req);
    ...
    // PUT: Güncelle
      ...
      try {
        const { first_name, last_name, email, message } = await parseRequestBody<ContactUser>(req);
        const result = await db.run(
          `UPDATE contact SET first_name = ?, last_name = ?, email = ?, message = ? WHERE id = ?`,
          [first_name, last_name, email, message, id]
        );
